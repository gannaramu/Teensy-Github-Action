...
for example in "${examples[@]}"; do
  echo -n $example:
  $HOME/arduino_ide/arduino-$ARDUINO_IDE_VERSION/arduino --verify --board "teensy:avr:$Device:usb=serial,speed=180,opt=o2std,keys=en-us" $example 2> error.txt > output.txt
